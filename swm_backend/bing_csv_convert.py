...
def main():
    frontend_data = []
    print("bing_csv_convert called from app.py")
    with open('bing_sample_output.csv', 'r', newline='') as inputFile, \
        ...
        for row in read_file:
            ...
            if routes and row_number!=0:
                ...
                # print(f"route_costs {route_costs}")
                routes_arr = (ast.literal_eval(routes))
                costs_arr = (ast.literal_eval(route_costs))
                for i in range(len(routes_arr)):
                    # print(f"costs arr {costs_arr}")
                    # print(f"routes_arr {routes_arr}")
                    truncated_cost = costs_arr  # round(float(costs_arr[i]), 2)
    ...
    # print(write_file_1)
    with open("bing_data_file.json", "w") as wf:
